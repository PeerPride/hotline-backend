...
module Orderable
  ...
  included do
    ...
    def move_up
      return if self.item_order == 1

      next_item = self.class.where(:item_order => self.item_order - 1).first
    ...
    def move_down
      return if self.item_order == self.class.max_order

      prev_item = self.class.where(:item_order => self.item_order + 1).first
